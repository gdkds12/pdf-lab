...
> 핵심 목표는 `GCS URI 입력 → (Router) Digital/Scanned 분기 → (Parse/OCR) 페이지 보존 전사 → Chunking(메타 포함) → Embedding(text-embedding-004) → OCI Supabase(chunks) 적재` 이다. [file:1]
...
- Infrastructure:
  ...
  - AI/ML:
    ...
    - Embedding: `text-embedding-004` (768d) [file:1]
...
### Step 5. Embedding (text-embedding-004, 768d)
- 모델: `text-embedding-004` [file:1]
