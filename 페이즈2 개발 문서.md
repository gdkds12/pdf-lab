...
## 4) 워커 설계(Cloud Run Job)
### 4.1 Job 입력
- `session_id`
- `audio_chunk_id` (권장: 하나의 Job이 하나의 chunk 처리)
- `gcs_chunk_url` 또는 `gs://...` URI(오디오 청크 객체)

### 4.2 처리 흐름(권장 고정)
1. DB에서 `audio_chunks` 로드 → `gcs_chunk_url`, `chunk_index`, `start_offset_sec` 확보 [file:1]
2. Gemini 호출로 신호 추출(JSON Schema 강제) [file:1]
3. 응답 검증(형식/의미) 후 `signals` INSERT
4. (선택) 다음 Phase 3 작업을 큐잉(예: evidence gathering job enqueue)
