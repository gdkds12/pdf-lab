...
### RPC: `hybrid_search_rrf(...)` (권장)
입력(권장):
- `p_query_text TEXT`
- `p_source_allowlist UUID[]` *(선택: 활성 교재만)*
- `p_k_vector INT`
- `p_k_keyword INT`
- `p_k_final INT`
- `p_rrf_c INT DEFAULT 60`
...
- `rrf_score FLOAT`
- (표시/인용용 메타) `page_start, page_end, anchor_path` *(Phase 4 블록 포맷 조립에 유리)*

***

## 성능/예산(현실적인 기본값)
초기 운영 기본값(추천):
- `k_vector = 30`, `k_keyword = 30`, `k_final = 50`
- 세션당 처리 쿼리 수가 많으면(예: signals가 수백 개) 쿼리 단위를 배치로 나눠 Cloud Run Job을 여러 번 실행한다.[1]
